            ...
            {coursesData.map((course, index) => (
              <div key={index} className="relative group cursor-target h-full" tabIndex={0} onClick={() => {}}>
                <div className="w-full h-full transition-transform duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] group-focus:[transform:rotateY(180deg)] group-focus-within:[transform:rotateY(180deg)]">
                  ...
                  {/* Back */}
                  <div className="absolute inset-0 w-full h-full bg-orange-50 border border-orange-200 rounded-xl overflow-y-auto flex flex-col items-center justify-start p-3 md:p-6 text-center shadow-md [backface-visibility:hidden] [transform:rotateY(180deg)] scrollbar-hide">
                    <h3 className="font-bold text-orange-800 text-xs md:text-base mb-1 md:mb-2 leading-tight flex-shrink-0">{course.title}</h3>
            ...
            {upcomingCoursesData.map((course, index) => (
              <div key={index} className="relative group cursor-target h-full" tabIndex={0} onClick={() => {}}>
                <div className="w-full h-full transition-transform duration-500 ease-in-out [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] group-focus:[transform:rotateY(180deg)] group-focus-within:[transform:rotateY(180deg)]">
                  ...
                  {/* Back */}
                  <div className="absolute inset-0 w-full h-full bg-gray-50 border border-gray-200 rounded-xl overflow-y-auto flex flex-col items-center justify-start p-3 md:p-6 text-center shadow-md [backface-visibility:hidden] [transform:rotateY(180deg)] scrollbar-hide">
                    <h3 className="font-bold text-gray-800 text-xs md:text-base mb-1 md:mb-2 leading-tight flex-shrink-0">{course.title}</h3>
                    <p className="text-gray-600 text-[10px] md:text-sm leading-snug md:leading-relaxed max-w-full md:max-w-[90%] flex-grow">
                      {course.description}
                    </p>
